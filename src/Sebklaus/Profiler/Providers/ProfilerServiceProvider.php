    ...
    protected function activateProfiler()
    {
        ...
        if ($this->app['config']->get('profiler::urlToggle')) {
            ...
            if ($this->app['session']->has('_profiler')) {
                $this->app['config']->set(
                    'profiler::profiler',
                    $this->app['session']->get('_profiler')
                );
            }
        }

        // Check console isn't running and profiler is enabled
        if (!$this->app->runningInConsole() && !$this->app['request']->ajax()) {
            $this->profiler = $this->app['config']->get('profiler::profiler');
        } else {
            $this->profiler = false;
        }
        ...
        $this->app['router']->after(function ($request, $response)
        {
            ...
            if (\Str::startsWith($response->headers->get('Content-Type'), 'text/html')) {
                ...
                if ($body_position !== false) {
                    $content = substr($content, 0, $body_position) .
                               $output .
                               substr($content, $body_position);
                } else {
                    $content .= $output;
                }
